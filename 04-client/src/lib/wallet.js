...
let _provider = new ethers.providers.JsonRpcProvider('https://sepolia.mode.network');
			...
			try {
				await wallet.request({
					...
					params: [{
						...
						rpcUrls: [CHAINDATA[919]['rpc']],
						nativeCurrency: {
							name: 'ETH',
							symbol: 'ETH',
							decimals: 18
						},
						blockExplorerUrls: [CHAINDATA[919]['explorer']]
					}],
				});
			} catch (addError) {
				// handle "add" error
